...
public class PacketRouter extends Router {
	...
	public PacketRouter(PacketRouter next, int bandwidth, int latency, int concurCoef) {
		...
		while (it.hasNext()) {
			...
			if (pdp.delay <= 0) {
				if (packetsQueue.size() < concurCoef)
					...
					StatisticsModule.getInstance().logLostReq(time, pdp.packet.source, pdp.packet.size);
				it.remove();
			}
		}
	}
	...
	private void forwardingUpdate(int time) {
		...
		if (current != null) {
			...
			if (transmittedBytes >= current.size) {
				if (next == null)
					StatisticsModule.getInstance().logReceivedReq(time + latency, current.source, current.received(time + latency), current.size);
				else
					forwardCurPacket();
			}
